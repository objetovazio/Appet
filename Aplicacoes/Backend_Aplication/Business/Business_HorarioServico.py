...
from Model.PeriodoAtividade import PeriodoAtividade as PA
from Model.HorarioServico import HorarioServico as HS
import sys
...
def createSchedule(schedule_data):
    try:
        periodo_assign = PA.PeriodoAtividade.get_by_id(
            schedule_data['period_id'])
        begin_time = time(schedule_data['begin_hour'][0:2],
                          schedule_data['begin_hour'][2:4], schedule_data['begin_hour'][4:6])
        end_time = time(schedule_data['end_hour'][0:2],
                        schedule_data['end_hour'][2:4], schedule_data['end_hour'][4:6])
...
def updateSchedule(schedule_data:dict):
    ...
    have_new_begin  = schedule_data['begin_time'] != None
    begin_time= schedule_data['begin_time'] if have_new_begin else old_schedule.horario_inicio

    have_new_end = schedule_data['end_time'] != None
    end_time = schedule_data['end_time'] if have_new_end else old_schedule.horario_fim
...
def findSchedule(query_param: dict):
    ...
    if (not is_period_empty):
        ...
        if(not is_day_empty):
            ...
            query_result = HS.HorarioServico.select().where((HS.HorarioServico.horario_fim == end_time))
...
def _makeResultDic(schedule_data):
    result = {
        ...
        'period_id':schedule_data.id_periodo.id_periodo,
        'week_day':schedule_data.dia_semana,
        'begin_time':schedule_data.horario_inicio,
        'end_time':schedule_data.horario_fim
    }
    return result
